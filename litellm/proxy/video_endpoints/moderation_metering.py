    ...
    return tuple(
        sorted(
            (
                ...
                *(window.counter_key for window in binding.windows),
            )
        )
    )
...
class MeteringStore:
    ...
    def from_client(cls, client: object, redis: cache.RedisCommands, *, namespace: str = "") -> MeteringStore:
        ...
        async def transactions() -> AsyncIterator[Database]:
            context = TypeAdapter(
                AbstractAsyncContextManager[object], config=ConfigDict(arbitrary_types_allowed=True)
            ).validate_python(factory())
    ...
    async def _identity(self, tx: Database, binding: BillingBinding) -> dict[str, Decimal]:
        rows = TypeAdapter(list[IdentityRow]).validate_python(
            await tx.query_raw(
        ...
        sources = (
            *(
                (("spend:end_user:" + binding.end_user_id, '"LiteLLM_EndUserTable"', "user_id", binding.end_user_id),)
                if binding.end_user_id
                else ()
            ),
            ...
            *(
                (
                    (
                        "spend:org:" + binding.organization_id,
        ...
        members = TypeAdapter(list[SpendRow]).validate_python(
            await tx.query_raw(
        ...
        for window in binding.windows:
            expected = binding.fingerprint if window.kind == "key" else binding.team_id
            if window.identity != expected or window.window_start >= window.reset_at:
                raise ValueError("window billing identity mismatch")
            await protected._balance(
                tx, BudgetIdentity(kind=window.kind, identity=window.identity, window=window.duration)
            )
        return {
    ...
    async def prepare(
        self, binding: BillingBinding, reservations: dict[str, Decimal], *, reservation_id: str | None = None
    ) -> None:
        ...
        try:
            async with self.transactions() as tx:
                ...
                old = TypeAdapter(list[TaskRow]).validate_python(
                    await tx.query_raw(
                        'SELECT binding,reservations,reservation_id FROM "LiteLLM_ModerationMeteringTask" WHERE intent_id=$1',
                        binding.intent_id,
                    )
                )
                if old:
                    if (
                        old[0].binding != binding
                        or old[0].reservations != reservations
                        or old[0].reservation_id != reservation_id
                    ):
                ...
                if reservation_id:
                    durable = TypeAdapter(tuple[ReservationState, ...]).validate_python(
                        await tx.query_raw(
                            "SELECT reservation_id,counter_key,generation,remaining,valid_until,status "
                            'FROM "LiteLLM_BudgetReservation" WHERE reservation_id=$1 FOR UPDATE',
                            reservation_id,
                        )
                    )
                    if {r.counter_key: r.remaining for r in durable if r.status == "active"} != reservations:
                        raise ValueError("durable reservation binding mismatch")
                    if not durable or any(r.counter_key not in balances for r in durable):
                        raise ValueError("durable reservation dimensions mismatch")
                await tx.execute_raw(
                    'INSERT INTO "LiteLLM_ModerationMeteringTask" (intent_id,binding,reservations,reservation_id) VALUES ($1,$2::jsonb,$3::jsonb,$4)',
                    binding.intent_id,
                    binding.model_dump_json(),
                    _json({key: str(value) for key, value in reservations.items()}),
                    reservation_id,
                )
                for phase in binding.expected_phases:
                    placeholder = PhaseEvent(
                        binding=binding,
                        request_id=request_id(binding, phase),
                        phase=phase,
                        provider="",
                        deployment_id="",
                        native_id="",
                    ...
                    await tx.execute_raw(
                        ...
                        "VALUES ($1,$2,$3,$4::jsonb,$5,'unknown')",
    ...
    async def persist(self, event: PhaseEvent) -> None:
        ...
        async with self.transactions() as tx:
            tasks = TypeAdapter(list[TaskRow]).validate_python(
                await tx.query_raw(
                    'SELECT binding,reservations,reservation_id FROM "LiteLLM_ModerationMeteringTask" WHERE intent_id=$1 FOR UPDATE',
                    event.binding.intent_id,
                )
            )
            if not tasks or tasks[0].binding != event.binding:
                raise ValueError("unattested metering event")
            siblings = TypeAdapter(list[PhaseRow]).validate_python(
                await tx.query_raw(
            ...
            if any(
                (
                    sibling.payload.provider,
                    sibling.payload.deployment_id,
                    sibling.payload.native_id,
                    sibling.payload.provider_task_id,
                )
                != (event.provider, event.deployment_id, event.native_id, event.provider_task_id)
            ...
            if old:
                ...
                await tx.execute_raw(
                    ...
                    event.digest(),
                    "pending" if event.finalized else "unknown",
                )
                return
            await tx.execute_raw(
                'INSERT INTO "LiteLLM_ModerationMeteringPhase" (request_id,intent_id,phase,payload,payload_hash,status) '
    ...
    async def quarantine(self, counter_keys: tuple[str, ...]) -> None:
        for key in counter_keys:
            await self.db.execute_raw(
                "UPDATE \"LiteLLM_ModerationMeteringCounter\" SET status='reconciliation_required',updated_at=NOW() WHERE counter_key=$1",
                key,
            )
    ...
    async def assert_admission(self, counter_keys: tuple[str, ...]) -> None:
        ...
        await ProtectedBudgetStore(self.db, self.transactions, self.redis, namespace=self.namespace).assert_admission(
            counter_keys
        )

    async def run_once(self) -> bool:
        token = uuid4().hex
        claimed = TypeAdapter(list[PhaseRow]).validate_python(
