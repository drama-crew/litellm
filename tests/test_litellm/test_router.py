...
async def test_video_generation_retry_releases_aggregate_but_retains_generation_gate():
    ...
    try:
        ...
        for task in (first, status, second):
            if not task.done():
                task.cancel()
        await asyncio.gather(first, status, second, return_exceptions=True)
        router.discard()
...
async def test_video_generation_cancelled_while_reacquiring_does_not_overrelease():
    ...
    try:
        ...
        await asyncio.gather(status, second, content)
    finally:
        status_release.set()
        second_create_release.set()
        for task in (status, second, content):
            if not task.done():
                task.cancel()
        await asyncio.gather(status, second, content, return_exceptions=True)
        router.discard()
