...
async def test_generic_async_failure_carries_deployment_metadata(monkeypatch):
    ...
    with pytest.raises(litellm.RateLimitError) as exc:
        await router._ageneric_api_call_with_fallbacks_helper(
            model="seedance-2.0", original_generic_function=failing
        )
    ...
    assert (
        Router._is_failover_eligible_exception(
            litellm.ContentPolicyViolationError(
                message="blocked", model="star-video2", llm_provider="libtv"
            )
        )
        is False
    )
    assert (
        Router._is_failover_eligible_exception(
            litellm.RateLimitError(
                message="capacity", model="star-video2", llm_provider="libtv"
            )
        )
        is True
    )
...
def test_video_aliases_resolve_only_via_explicit_video_method():
    ...
    assert (
        router.resolve_video_model_id_alias("libtv", "seedance-2.0")
        == "libtv-seedance-2-standard-account-1"
    )
    assert (
        router.resolve_video_model_id_alias("libtv", "_fallback2/seedance-2.0")
        == "libtv-seedance-2-standard-account-2"
    )
