...
@pytest.mark.asyncio
async def test_track_cost_callback_does_not_compete_with_causyn_billing_outbox():
    logger = _ProxyDBLogger()
    kwargs = {
        "model": "causyn-1.0",
    ...
    with (
        ...
        patch(
            "litellm.proxy.hooks.proxy_track_cost_callback._release_budget_reservation",
            new_callable=AsyncMock,
        ) as release_budget,
    ):
        mock_proxy_logging.db_spend_update_writer = MagicMock()
        ...
        release_budget.assert_awaited_once()
        mock_proxy_logging.db_spend_update_writer.update_database.assert_not_called()
